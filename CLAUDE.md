...
## Источник содержания курса

Программа, занятия, ДЗ и артефакты живут не здесь, а в отдельном проекте `~/Documents/Cursor/AI Agent Bootcamp` (репозиторий `dkoraitest/ai-agent-bootcamp`). Этот репозиторий — только платформа. Прежде чем менять любой текст программы, названия занятий, формулировки ДЗ или критерии, читай источник и сверяйся с датой последнего коммита: программа переписывается между потоками.

Что где лежит в проекте содержания:

| Файл | Что даёт |
|---|---|
| `VC_Bootcamp/lessons_v2.csv` | занятия: неделя, номер, название, цель, дата, номер ДЗ |
| `VC_Bootcamp/assignments_v2.csv` | ДЗ: название, что сдаём, пир-ревью, дедлайн словами |
| `VC_Bootcamp/01_PROGRAM.html` | развёрнутая программа: тайминг занятий, критерии ДЗ, темы недель |
| `VC_Bootcamp/teaching_kits/` | брифы спикера, теория, домашки по неделям |
| `VC_Bootcamp/teaching_kits/ARTIFACTS.md` | инвентаризация артефактов: что есть, что делаем |
| `VC_Bootcamp/templates/` | шаблоны и учебные данные (`svodka/`, `TPL1_claude_md.md`, хуки) |
| `VC_Bootcamp/12_RETRO.html` | разбор прошедшего потока |

Внутри самих документов бывают расхождения: CSV обновляют, а HTML-блоки не всегда. При конфликте свежее то, что менялось последним коммитом — проверяй через `git log -p`.

Программа второго потока (v2, «вайб кодинг») — коммит `356a9e4` от 06.08.2026: 6 недель, 12 занятий, 6 ДЗ, 06.08–15.09.2026.

### Как подтянуть изменившуюся программу
...
1. печатает занятия, темы недель и ДЗ так, как они заданы в источнике;
2. показывает, где зашитый в интерфейс текст разошёлся с программой;
3. отдаёт idempotent-SQL на `cohort_lesson_schedule`, лишние занятия и `cohorts.ends_at`.

В базу он ничего не пишет: SQL применяется через Supabase MCP или SQL Editor, чтобы изменение программы проходило через глаза человека. Не трогает и то, что скриптом брать неоткуда:

- **время занятий** (`starts_at`) — в источнике только даты, время задаётся на `/admin/schedule`;
- **дедлайны ДЗ** — в источнике они словами («Пн 12:00 после W1»), в даты их разворачивает человек;
- **тексты ДЗ и техники занятий** — зашиты в `page.tsx`, `program/page.tsx`, `assignments/page.tsx`, скрипт только сообщает о расхождении.
...
### Материалы и артефакты ДЗ

Материалы лежат в `materials`, видимость и ссылка на поток — в `cohort_material_settings` (её `url` перекрывает `materials.url`). Какие материалы показывать в карточке ДЗ, задаёт `src/lib/program/hwMaterials.ts`: номер ДЗ → список id материалов. Скрытый для потока материал в карточку не попадёт.

Небольшие файлы-артефакты (учебные данные, шаблоны) лежат в `public/artifacts/` и раздаются самим приложением — ссылка вида `/artifacts/svodka.zip`. Так участник скачивает их из LMS, не уходя на внешние диски.
